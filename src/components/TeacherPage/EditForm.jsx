...
const EditForm = ({ assignment, students, teachers, subjects, occupiedStudents, onSubmit, onClose }) => {
  ...
  useEffect(() => {
    if (assignment) {
      const subject = (subjects || []).find(s => s.id === assignment.subject_id || s.name === assignment.subject);
      const student = (students || []).find(s => s.id === assignment.student_id);

      setStudentId(student ? { label: student.full_name, value: student.id } : null);
  ...
  const studentOptions = useMemo(() => {
    const currentStudentId = assignment?.student_id;
    return (students || []).map(s => {
      const isBusy = (occupiedStudents || []).includes(s.id) && s.id !== currentStudentId;
      return {
  ...
  }, [students, occupiedStudents, assignment?.student_id]);
